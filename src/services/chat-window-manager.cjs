...
class ChatWindowManager {
  constructor({ userDataFile, preloadPath, rendererPath, onClosed } = {}) {
    this.userDataFile = userDataFile;
    this.preloadPath = preloadPath;
    this.rendererPath = rendererPath;
    this.onClosed = onClosed || (() => {});
    this.window = null;
    this.settings = { x:null, y:null, width:560, height:760, alwaysOnTop:false };
  }
  async load(){
    try { this.settings = { ...this.settings, ...JSON.parse(await fs.readFile(this.userDataFile,"utf8")) }; } catch {}
    return this.settings;
  }
  async save(){
    await fs.mkdir(path.dirname(this.userDataFile),{recursive:true});
    await fs.writeFile(this.userDataFile,JSON.stringify(this.settings,null,2),"utf8");
  }
  create(){
    if(this.window && !this.window.isDestroyed()){ this.window.show(); this.window.focus(); return this.window; }
    const display = screen.getPrimaryDisplay();
    const work = display.workArea;
    const x = Number.isFinite(this.settings.x) ? this.settings.x : work.x + Math.max(0,Math.round((work.width-this.settings.width)/2));
    const y = Number.isFinite(this.settings.y) ? this.settings.y : work.y + Math.max(0,Math.round((work.height-this.settings.height)/2));
