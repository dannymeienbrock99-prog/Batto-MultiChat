...
class ChatCore extends EventEmitter{
  ...
  normalize(input={}){
    ...
    return{
      ...
      color:/^#(?:[\da-f]{3}|[\da-f]{6})$/i.test(String(input.color||""))?String(input.color):"",
      ...
      metrics:{likeCount:Number(input.metrics?.likeCount??input.likeCount??metadata.likeCount??metadata.like_count??0)||0,totalLikeCount:Number(input.metrics?.totalLikeCount??input.totalLikeCount??metadata.totalLikeCount??metadata.total_like_count??0)||0,viewerCount:Number(input.metrics?.viewerCount??input.viewerCount??metadata.viewerCount??metadata.viewer_count??metadata.userCount??0)||0},
      metadata
    };
  }
