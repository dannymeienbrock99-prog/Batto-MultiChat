...
class EulerOAuth{
  ...
  async config(){const s=await this.settingsStore.get("tiktokOAuth");return s||{};}
  async begin({clientId,clientSecret,scopes=DEFAULT_SCOPES}={}){
    ...
    const state=crypto.randomBytes(24).toString("hex");const redirectUri=`http://127.0.0.1:${this.port}/oauth/tiktok/callback`;
  ...
  async requestToken(body){const response=await fetch(TOKEN_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error_description||data.message||`OAuth HTTP ${response.status}`);return data;}
  async exchangeCode(code,redirectUri){const cfg=await this.config();const clientSecret=await this.secretStore.get("euler.clientSecret");if(!clientSecret)throw new Error("Euler OAuth Client Secret fehlt.");const data=await this.requestToken({grant_type:"authorization_code",code,client_id:cfg.clientId,client_secret:clientSecret,redirect_uri:redirectUri});await this.saveTokens(data);return this.status();}
  async refresh(){const cfg=await this.config();const clientSecret=await this.secretStore.get("euler.clientSecret");const refreshToken=await this.secretStore.get("euler.refreshToken");if(!refreshToken)throw new Error("Kein TikTok Refresh Token vorhanden.");const data=await this.requestToken({grant_type:"refresh_token",refresh_token:refreshToken,client_id:cfg.clientId,client_secret:clientSecret});await this.saveTokens(data);return this.status();}
  async saveTokens(data){if(data.access_token)await this.secretStore.set("euler.accessToken",data.access_token);if(data.refresh_token)await this.secretStore.set("euler.refreshToken",data.refresh_token);await this.settingsStore.set("tiktokSession",{scopes:data.scopes||data.scope||[],accessExpiresAt:Date.now()+Number(data.expires_in||3600)*1000,refreshExpiresAt:Date.now()+Number(data.refresh_expires_in||2592000)*1000,updatedAt:Date.now()});}
  async accessToken(){const session=await this.settingsStore.get("tiktokSession")||{};if(session.accessExpiresAt&&Date.now()>session.accessExpiresAt-120000)await this.refresh();return this.secretStore.get("euler.accessToken");}
  ...
  async status(){const session=await this.settingsStore.get("tiktokSession")||{};const token=await this.secretStore.get("euler.accessToken");return{connected:Boolean(token),scopes:session.scopes||[],accessExpiresAt:session.accessExpiresAt||0,refreshExpiresAt:session.refreshExpiresAt||0};}
}
